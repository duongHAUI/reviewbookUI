<template>
  ...
  >
    <label for="m-input" class="m-lable m-lable-cbb" v-if="textField"
      >{{ textField }}<span>{{ required && textField ? ' *' : '' }}</span></label
    >
    <div class="m-input-main m-input-main-cbb">
      ...
      <div class="m-combobox__btn" @click="onToggleList" @keydown="inputOnKeyDown">
        <div
...
export default {
  ...
  created() {
    ...
    if (this.locationList == enumD.LoCATION_LIST_CBB.UP) {
      this.locationSize = 35 * this.data.length + 37
    }
  },
  methods: {
    /**
    ...
    onToggleList() {
      ...
      if (this.textSelected) {
        this.indexItemSelected = this.dataSearch.findIndex(
          (x) => x[this.propName].toLowerCase() == this.textSelected.toLowerCase()
        )
      }
    },
    /**
    ...
    itemSelected(item, index) {
      ...
      this.$emit('selected', item[this.propValue])
    },
    /**
    ...
    checkValidate() {
      if (this.rules.length != 0) {
        ...
        if (
          ...
          msgErrorInput = common.inputValidation(this.rules, this.name, this.modelValue)
        }
